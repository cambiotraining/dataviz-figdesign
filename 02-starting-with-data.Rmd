...
Now it is time for you to practice some of these things. Try to do the following:

1. Download the data into the `data_raw` folder within your working directory
2. Read in the data using the `read_csv()` function and store it in an object `surveys`
3. Find out what data types are in `surveys`
4. Are there missing values in the data? If so, can you see how many?
...
:::exercise
### Exercise - Boxplot
The plot we created above was a simple scatter plot, which used the `geom_point()` function. There are many different types of geoms in ggplot and to explore this we would like you to create a [box plot](https://en.wikipedia.org/wiki/Box_plot), using the same data set.
